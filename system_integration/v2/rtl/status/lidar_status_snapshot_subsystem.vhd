...
-- Single K0-8 runtime-status owner. Only an 8-bit request and one atomic
-- 33-bit {error,data} response cross each clock boundary. Large diagnostic
-- records remain in their native domains and are never sampled bit by bit.
entity lidar_status_snapshot_subsystem is
    generic (
...
end entity lidar_status_snapshot_subsystem;

architecture rtl of lidar_status_snapshot_subsystem is
...
begin
    ...
    u_proc_status : entity work.lidar_processing_status_source
        port map (
            i_clk => i_proc_clk,

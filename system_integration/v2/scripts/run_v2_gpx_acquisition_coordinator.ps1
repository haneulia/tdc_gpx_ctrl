...
$WorkRoot = Join-Path $Hdl "tmp/v2_gpx_acquisition_coordinator"
$Work = Join-Path $WorkRoot $Stamp
$Archive = Join-Path $Hdl "signoff_results/sessions/${Stamp}_v2_gpx_acquisition_coordinator"
...
$SimFiles = $CommonFiles + @(
    ...
    "$Hdl/system_integration/v2/tb/tb_lidar_gpx_acquisition_coordinator.vhd"
)
$SynthFiles = $CommonFiles + @(
    "$Hdl/system_integration/v2/tb/lidar_gpx_acquisition_coordinator_impl.vhd"
)

$Project = Join-Path $Work "v2_gpx_acquisition_coordinator_vhdl.prj"
$VerilogProject = Join-Path $Work "v2_gpx_acquisition_coordinator_verilog.prj"
...
$SimulationProfiles = @(
    [ordered]@{
        top = "tb_lidar_gpx_event_merge"
        marker = "LIDAR_V2_GPX_EVENT_MERGE_PASS"
    },
    [ordered]@{
        top = "tb_lidar_gpx_acquisition_coordinator_150"
        marker = "LIDAR_V2_GPX_ACQUISITION_COORDINATOR_PASS"
    },
    [ordered]@{
        top = "tb_lidar_gpx_acquisition_coordinator_200"
        marker = "LIDAR_V2_GPX_ACQUISITION_COORDINATOR_PASS"
    }
)
...
if (-not $SkipSimulation) {
    ...
    try {
        ...
        foreach ($Profile in $SimulationProfiles) {
            $Top = $Profile.top
            ...
            if ((Get-Content -Raw -LiteralPath $SimLog) -notmatch
                [regex]::Escape($Profile.marker)) {
                throw "$Top did not report the final PASS marker"
            }
            if ((Get-Content -Raw -LiteralPath $SimLog) -match
                'Failure:|Fatal:') {
...
if (-not $SkipImplementation) {
    foreach ($Profile in $ImplementationProfiles) {
        ...
        @(
            ...
            "synth_design -top lidar_gpx_acquisition_coordinator_impl -part xc7z020clg484-2 -mode out_of_context -flatten_hierarchy none -generic G_TDC_CLK_MHZ=$($Profile.mhz)",
...
$Scenario = [ordered]@{
    checkpoint = "H2B-2"
    chip_count = 4
    runtime_active_mask = "1011"
    echo_delay_profile = "CTL20 CH0 + channel * STEP"
    simulation_profiles = $SimulationProfiles
    implementation_profiles = @($ImplementationProfiles)
    contracts = @(
        "atomic active-lane Shot broadcast",
        "registered round-robin event merge",
        "inactive lane performs no acquisition",
        "all active terminal events before completion"
    )
}
...
Write-Output "LIDAR_V2_GPX_ACQUISITION_COORDINATOR_PASS"

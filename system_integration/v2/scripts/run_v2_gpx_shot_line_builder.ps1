...
$WorkRoot = Join-Path $Hdl "tmp/v2_gpx_shot_line_builder"
$Work = Join-Path $WorkRoot $Stamp
$Archive = Join-Path $Hdl (
    "signoff_results/sessions/${Stamp}_v2_gpx_shot_line_builder")
...
$SimFiles = $CommonFiles + @(
    "$Hdl/system_integration/v2/tb/tb_lidar_gpx_shot_line_builder.vhd"
)

$Project = Join-Path $Work "v2_gpx_shot_line_builder_vhdl.prj"
...
$SimulationProfiles = @(
    "tb_lidar_gpx_shot_line_builder_150",
    "tb_lidar_gpx_shot_line_builder_200"
)
    ...
    try {
        ...
        foreach ($Top in $SimulationProfiles) {
            ...
            if ((Get-Content -Raw -LiteralPath $SimLog) -notmatch
                "LIDAR_V2_GPX_SHOT_LINE_BUILDER_PASS") {
...
if (-not $SkipImplementation) {
    foreach ($Profile in $ImplementationProfiles) {
        ...
        @(
            ...
            "synth_design -top lidar_gpx_shot_line_builder -part xc7z020clg484-2 -mode out_of_context -flatten_hierarchy none",
...
$Scenario = [ordered]@{
    checkpoint = "J5/B9 Shot Metadata and canonical line words"
    processing_clocks_mhz = @(150, 200)
    metadata_bytes = 16
    timestamp_bits = 64
    output_word_bits = 32
    backpressure = "registered output event remains stable"
...
Write-Output "LIDAR_V2_GPX_SHOT_LINE_BUILDER_PASS"

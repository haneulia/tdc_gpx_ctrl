...
$WorkRoot = Join-Path $Hdl "tmp/v2_motor_position"
$Work = Join-Path $WorkRoot $Stamp
$Archive = Join-Path $Hdl `
    "signoff_results/sessions/${Stamp}_v2_motor_position"
...
$SimFiles = @(
    ...
    $SynthFiles[4],
    $SynthFiles[5],
    "$Hdl/system_integration/v2/tb/tb_motor_position_core.vhd",
    "$Hdl/system_integration/v2/tb/tb_motor_position_core_profiles.vhd"
)

$Project = Join-Path $Work "v2_motor_position.prj"
...
$SimProfiles = @(
    [ordered]@{
        name = "150mhz"
        top = "tb_motor_position_core_150"
        marker = "LIDAR_V2_MOTOR_POSITION_PASS proc_mhz=150"
    },
    [ordered]@{
        name = "200mhz"
        top = "tb_motor_position_core_200"
        marker = "LIDAR_V2_MOTOR_POSITION_PASS proc_mhz=200"
    }
)
...
try {
    ...
    foreach ($Profile in $SimProfiles) {
        $Snapshot = "v2_motor_$($Profile.name)_${Stamp}_snap"
        ...
        foreach ($Scenario in "P00", "P01", "P02", "P03", "P04") {
            if ($Text -notmatch "V2-MOTOR-$Scenario.*PASS") {
...
foreach ($Profile in $ImplProfiles) {
    ...
    @(
        ...
        "synth_design -top motor_position_core -part xc7z020clg484-2 -mode out_of_context -flatten_hierarchy rebuilt",
        ...
        "if {[llength `$async_regs] < 12} { error {Physical synchronizer attributes missing} }",
        "exit"
...
Write-Output "LIDAR_V2_MOTOR_POSITION_PASS"

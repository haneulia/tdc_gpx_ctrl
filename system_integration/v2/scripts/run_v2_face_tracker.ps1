...
$WorkRoot = Join-Path $Hdl "tmp/v2_face_tracker"
$Work = Join-Path $WorkRoot $Stamp
$Archive = Join-Path $Hdl `
    "signoff_results/sessions/${Stamp}_v2_face_tracker"
...
$SimFiles = @(
    ...
    $SynthFiles[2],
    $SynthFiles[3],
    "$Hdl/system_integration/v2/tb/tb_face_tracker.vhd",
    "$Hdl/system_integration/v2/tb/tb_face_tracker_profiles.vhd"
)

$Project = Join-Path $Work "v2_face_tracker.prj"
...
try {
    ...
    foreach ($Mhz in 150, 200) {
        foreach ($Faces in 1..5) {
            $Name = "${Mhz}mhz_f${Faces}"
            $Top = "tb_face_tracker_${Mhz}_f${Faces}"
            $Snapshot = "v2_face_${Name}_${Stamp}_snap"
            ...
            $Marker = "LIDAR_V2_FACE_TRACKER_PASS proc_mhz=$Mhz faces=$Faces"
            if ($Text -notmatch [regex]::Escape($Marker)) {
                throw "$Name did not report PASS"
            }
            foreach ($Scenario in "P10", "P11", "P12", "P13") {
                if ($Text -notmatch "V2-FACE-$Scenario.*PASS") {
...
foreach ($Profile in $ImplProfiles) {
    ...
    @(
        ...
        "synth_design -top face_tracker -part xc7z020clg484-2 -mode out_of_context -flatten_hierarchy rebuilt",
...
Write-Output "LIDAR_V2_FACE_TRACKER_PASS"

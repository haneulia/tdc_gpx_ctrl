...
$WorkRoot = Join-Path $Hdl "tmp/v2_gpx_face_footer"
$Work = Join-Path $WorkRoot $Stamp
$Archive = Join-Path $Hdl (
    "signoff_results/sessions/${Stamp}_v2_gpx_face_footer")
...
$CommonFiles = @(
    ...
    "$Hdl/system_integration/v2/rtl/proc/lidar_gpx_frame_close_fork.vhd",
...
$ImplFiles = $CommonFiles + @(
    "$Hdl/system_integration/v2/tb/lidar_gpx_face_footer_builder_impl.vhd"
)
$SimFiles = $CommonFiles + @(
    "$Hdl/system_integration/v2/tb/tb_lidar_gpx_face_footer_builder.vhd",
    "$Hdl/system_integration/v2/tb/tb_lidar_gpx_vdma_profile_manager.vhd",
    "$Hdl/system_integration/v2/tb/tb_lidar_gpx_frame_close_fork.vhd"
)
...
if (-not $SkipSimulation) {
    $Project = Join-Path $Work "v2_gpx_face_footer_vhdl.prj"
    ...
    try {
        ...
        foreach ($Mhz in $ClockValues) {
            foreach ($Width in $WidthValues) {
                $Top = "tb_lidar_gpx_face_footer_builder_${Mhz}_${Width}"
                ...
                if ((Get-Content -Raw -LiteralPath $SimLog) -notmatch
                    "LIDAR_V2_GPX_FACE_FOOTER_PASS") {
                    throw "$Top did not report the final PASS marker"
                }

                $Top = "tb_lidar_gpx_vdma_profile_manager_${Mhz}_${Width}"
                $Snapshot = "${Top}_${Stamp}_snap"
                $SimLog = Join-Path $Work "xsim_${Top}.log"
                Invoke-Checked "$Vivado/xelab.bat" @(
                    "--debug", "off", "--relax", "--mt", "2",
                ...
                if ((Get-Content -Raw -LiteralPath $SimLog) -notmatch
                    "LIDAR_V2_GPX_VDMA_PROFILE_MANAGER_PASS") {
                    throw "$Top did not report the final PASS marker"
                }
            }
            $Top = "tb_lidar_gpx_frame_close_fork_${Mhz}"
            $Snapshot = "${Top}_${Stamp}_snap"
            $SimLog = Join-Path $Work "xsim_${Top}.log"
            Invoke-Checked "$Vivado/xelab.bat" @(
                "--debug", "off", "--relax", "--mt", "2",
            ...
            if ((Get-Content -Raw -LiteralPath $SimLog) -notmatch
                "LIDAR_V2_GPX_FRAME_CLOSE_FORK_PASS") {
                throw "$Top did not report the final PASS marker"
            }
        }
    }
...
if (-not $SkipImplementation) {
    foreach ($Mhz in $ClockValues) {
        foreach ($Width in $WidthValues) {
            ...
            $ReadFiles = ($ImplFiles | ForEach-Object {
                "{$($_.Replace('\', '/'))}"
            }) -join " "
            ...
            @(
                ...
                "synth_design -top lidar_gpx_face_footer_builder_impl -part xc7z020clg484-2 -mode out_of_context -flatten_hierarchy none -generic G_CLK_MHZ=$Mhz -generic G_OUTPUT_WIDTH=$Width",
                "create_clock -name proc_clk -period $Period [get_ports i_clk]",
                "opt_design",
                "place_design",
                "phys_opt_design",
                "route_design",
...
$Scenario = [ordered]@{
    checkpoint = "J7/J8 Face Footer and Face-boundary VDMA profile activation"
    processing_clocks_mhz = @($ClockValues)
    output_widths = @($WidthValues)
    planned_shots = 2
    minimum_cell_profile = "1 Cell / 1 visible Return"
    footer = "8 canonical Words; split using active HSIZE"
    profile_activation = "calculate pending; safe boundary; VDMA ACK; Active"
    backpressure = "registered Footer and AXIS outputs remain stable"
}
$Scenario | ConvertTo-Json -Depth 5 | Set-Content -Encoding ASCII -LiteralPath (
    Join-Path $Archive "scenario.json")

$Manifest = foreach ($File in @($SimFiles + $ImplFiles + $PSCommandPath) |
        Sort-Object -Unique) {
...
Write-Output "LIDAR_V2_GPX_FACE_FOOTER_PASS"

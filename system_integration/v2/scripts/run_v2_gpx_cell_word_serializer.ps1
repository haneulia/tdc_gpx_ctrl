...
$WorkRoot = Join-Path $Hdl "tmp/v2_gpx_cell_word_serializer"
$Work = Join-Path $WorkRoot $Stamp
$Archive = Join-Path $Hdl (
    "signoff_results/sessions/${Stamp}_v2_gpx_cell_word_serializer")
...
$SimFiles = $CommonFiles + @(
    "$Hdl/system_integration/v2/tb/tb_lidar_gpx_cell_word_serializer.vhd"
)

$Project = Join-Path $Work "v2_gpx_cell_word_serializer_vhdl.prj"
...
$SimulationProfiles = @(
    "tb_lidar_gpx_cell_word_serializer_150",
    "tb_lidar_gpx_cell_word_serializer_200"
)
    ...
    try {
        ...
        foreach ($Top in $SimulationProfiles) {
            ...
            if ((Get-Content -Raw -LiteralPath $SimLog) -notmatch
                "LIDAR_V2_GPX_CELL_WORD_SERIALIZER_PASS") {
...
if (-not $SkipImplementation) {
    foreach ($Profile in $ImplementationProfiles) {
        ...
        @(
            ...
            "synth_design -top lidar_gpx_cell_word_serializer -part xc7z020clg484-2 -mode out_of_context -flatten_hierarchy none",
...
$Scenario = [ordered]@{
    checkpoint = "J1/B9 Cell serializer"
    processing_clocks_mhz = @(150, 200)
    max_hits = @(1, 2, 3, 4, 5, 6, 7)
    slopes = @("Rise", "Fall")
    blank_cell = $true
    hit_width = 17
    canonical_word_width = 32
    backpressure = "registered output holds full word event"
    continuous_cell_boundary = "last word load overlaps next Cell accept"
...
Write-Output "LIDAR_V2_GPX_CELL_WORD_SERIALIZER_PASS"

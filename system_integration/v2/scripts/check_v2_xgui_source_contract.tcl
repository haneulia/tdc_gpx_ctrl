...
proc v2_check_xgui_source_contract {xgui_path} {
    ...
    if {![string match {# Definitional proc to organize widgets*} $source_text]} {
        error {V2 XGUI is not in Vivado-native generated form}
    }
    if {[string first {namespace eval} $source_text] >= 0 ||
        ...
        error {V2 XGUI contains hand-maintained callback logic}
    }
    ...
    if {$init_position < 0 || $callback_position <= $init_position} {
        error {V2 XGUI must place explicit init_gui before generated callbacks}
    }
    set init_text [string range $source_text $init_position \
        [expr {$callback_position - 1}]]
    if {[string first {foreach } $init_text] >= 0 ||
        [string first {eval } $init_text] >= 0} {
        error {V2 XGUI init_gui must not generate Layout items dynamically}
    }
    ...
    foreach required_layout {
        ...
        if {[string first $required_layout $init_text] < 0} {
            error "V2 XGUI visual Layout item is missing: $required_layout"
        }
    }
    ...
    foreach generic $generics {
        foreach prefix {
            ...
            if {[string first $declaration $source_text] < 0} {
                error "V2 XGUI callback is missing: $prefix$generic"
            }
        }
    }
    ...
    foreach generic $generics {
        ...
        if {[string first {set_property} $update_body] >= 0 ||
            ...
            error "V2 XGUI parameter callback was hand edited: $generic"
        }
        if {[string first {return true} $validate_body] < 0} {
            error "V2 XGUI generated validation stub is malformed: $generic"
        }
        ...
        if {[string first $expected $model_body] < 0} {
            error "V2 XGUI model-parameter mapping is not direct: $generic"
        }
    }
    interp delete $xgui_interp
    puts {LIDAR_V2_XGUI_NATIVE_VISUAL_CONTRACT_PASS}
}

if {[file normalize [info script]] eq [file normalize $argv0]} {
    if {[llength $argv] != 1} {
        error {Usage: check_v2_xgui_source_contract.tcl <xgui.tcl>}
    }
    v2_check_xgui_source_contract [file normalize [lindex $argv 0]]
}

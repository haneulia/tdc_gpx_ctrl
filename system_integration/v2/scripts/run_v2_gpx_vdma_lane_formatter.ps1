...
$WorkRoot = Join-Path $Hdl "tmp/v2_gpx_vdma_lane_formatter"
$Work = Join-Path $WorkRoot $Stamp
$Archive = Join-Path $Hdl (
    "signoff_results/sessions/${Stamp}_v2_gpx_vdma_lane_formatter")
...
$CommonFiles = @(
    ...
    "$Hdl/system_integration/v2/rtl/proc/lidar_gpx_vdma_lane_formatter.vhd"
)
$SimFiles = $CommonFiles + @(
    "$Hdl/system_integration/v2/tb/tb_lidar_gpx_vdma_lane_formatter.vhd"
)

$Project = Join-Path $Work "v2_gpx_vdma_lane_formatter_vhdl.prj"
...
$SimulationProfiles = @(
    "tb_lidar_gpx_vdma_lane_formatter_32_150",
    "tb_lidar_gpx_vdma_lane_formatter_64_150",
    "tb_lidar_gpx_vdma_lane_formatter_128_150",
    "tb_lidar_gpx_vdma_lane_formatter_32_200",
    "tb_lidar_gpx_vdma_lane_formatter_64_200",
    "tb_lidar_gpx_vdma_lane_formatter_128_200"
)
if ($Only200MHz) {
    $SimulationProfiles = @($SimulationProfiles | Where-Object {
        $_ -match '_200$'
    })
}
    ...
    try {
        ...
        foreach ($Top in $SimulationProfiles) {
            ...
            if ((Get-Content -Raw -LiteralPath $SimLog) -notmatch
                "LIDAR_V2_GPX_VDMA_LANE_FORMATTER_PASS") {
...
$ImplementationProfiles = @(
    [ordered]@{ name = "w32_150mhz"; width = 32; mhz = 150; period = "6.667" },
    [ordered]@{ name = "w64_150mhz"; width = 64; mhz = 150; period = "6.667" },
    [ordered]@{ name = "w128_150mhz"; width = 128; mhz = 150; period = "6.667" },
    [ordered]@{ name = "w32_200mhz"; width = 32; mhz = 200; period = "5.000" },
    [ordered]@{ name = "w64_200mhz"; width = 64; mhz = 200; period = "5.000" },
    [ordered]@{ name = "w128_200mhz"; width = 128; mhz = 200; period = "5.000" }
)
if ($Only200MHz) {
...
if (-not $SkipImplementation) {
    foreach ($Profile in $ImplementationProfiles) {
        ...
        @(
            ...
            "synth_design -top lidar_gpx_vdma_lane_formatter -part xc7z020clg484-2 -mode out_of_context -flatten_hierarchy none -generic {g_OUTPUT_WIDTH=$($Profile.width)}",
...
$Scenario = [ordered]@{
    checkpoint = "J2/B9 single-lane VDMA formatter"
    processing_clocks_mhz = @(150, 200)
    output_widths = @(32, 64, 128)
    slot_counts = @(1, 3, 5, 32)
    cell_word_counts = @(2, 3, 4, 5)
    prefix_bytes = 48
    line_alignment_bytes = 16
    backpressure = "full AXIS payload and sideband stability"
    abort = "capture cancel; emitted line drains through EOL and reports fault"
...
$ManifestFiles = @($SimFiles + $PSCommandPath) | Sort-Object -Unique
...
Write-Output "LIDAR_V2_GPX_VDMA_LANE_FORMATTER_PASS"

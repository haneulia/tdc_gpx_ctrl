...
$WorkRoot = Join-Path $Hdl "tmp/v2_config_manager"
$Work = Join-Path $WorkRoot $Stamp
$Archive = Join-Path $Hdl `
    "signoff_results/sessions/${Stamp}_v2_config_manager"
...
$SimFiles += @(
    "$Hdl/system_integration/v2/tb/tb_lidar_config_gateway.vhd",
    "$Hdl/system_integration/v2/tb/tb_lidar_config_subsystem.vhd",
    "$Hdl/system_integration/v2/tb/tb_lidar_config_subsystem_profiles.vhd"
)

$Project = Join-Path $Work "v2_config_manager.prj"
...
$SimProfiles = @(
    [ordered]@{
        name = "gateway"
        top = "tb_lidar_config_gateway"
        marker = "LIDAR_V2_CONFIG_GATEWAY_PASS"
    },
    [ordered]@{
        name = "proc150_tdc200"
        top = "tb_lidar_config_subsystem_150_200"
        marker = "LIDAR_V2_CONFIG_SUBSYSTEM_PASS proc_mhz=150 tdc_mhz=200"
    },
    [ordered]@{
        name = "proc200_tdc150"
        top = "tb_lidar_config_subsystem_200_150"
        marker = "LIDAR_V2_CONFIG_SUBSYSTEM_PASS proc_mhz=200 tdc_mhz=150"
    }
)
...
try {
    ...
    foreach ($Profile in $SimProfiles) {
        $Snapshot = "v2_mgr_$($Profile.name)_${Stamp}_snap"
...
foreach ($Profile in $ImplProfiles) {
    ...
    @(
        ...
        "synth_design -top lidar_config_subsystem -part xc7z020clg484-2 -mode out_of_context -flatten_hierarchy rebuilt",
...
Write-Output "LIDAR_V2_CONFIG_MANAGER_PASS"

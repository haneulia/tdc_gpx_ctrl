...
$WorkRoot = Join-Path $Hdl "tmp/v2_laser_executor"
$Work = Join-Path $WorkRoot $Stamp
$Archive = Join-Path $Hdl `
    "signoff_results/sessions/${Stamp}_v2_laser_executor"
...
$SimFiles = @(
    ...
    $SynthFiles[6],
    $SynthFiles[7],
    "$Hdl/system_integration/v2/tb/tb_laser_executor.vhd",
    "$Hdl/system_integration/v2/tb/tb_laser_executor_profiles.vhd",
    "$Hdl/system_integration/v2/tb/tb_v2_laser_control_chain.vhd",
    "$Hdl/system_integration/v2/tb/tb_v2_laser_control_chain_profiles.vhd"
)

$Project = Join-Path $Work "v2_laser_executor.prj"
...
try {
    ...
    foreach ($Mhz in 150, 200) {
        $UnitTop = "tb_laser_executor_$Mhz"
        $UnitSnapshot = "v2_laser_${Mhz}_${Stamp}_snap"
        ...
        foreach ($Scenario in "P30", "P31", "P32", "P33", "P34", "P36") {
            if ($UnitText -notmatch "V2-LASER-$Scenario.*PASS") {
                throw "${Mhz} MHz unit test did not pass $Scenario"
            }
        }
        $UnitMarker = "LIDAR_V2_LASER_EXECUTOR_PASS proc_mhz=$Mhz"
        if ($UnitText -notmatch [regex]::Escape($UnitMarker)) {
            throw "${Mhz} MHz unit test did not report PASS"
        }

        $ChainTop = "tb_v2_laser_control_chain_$Mhz"
        $ChainSnapshot = "v2_laser_chain_${Mhz}_${Stamp}_snap"
        ...
        if ($ChainText -notmatch "V2-LASER-P35.*PASS") {
            throw "${Mhz} MHz chain test did not pass P35"
        }
        $ChainMarker = "LIDAR_V2_LASER_CONTROL_CHAIN_PASS proc_mhz=$Mhz"
...
foreach ($Profile in $ImplProfiles) {
    ...
    @(
        ...
        "synth_design -top laser_executor -part xc7z020clg484-2 -mode out_of_context -flatten_hierarchy rebuilt",
        ...
        "if {[llength `$async_regs] != 4} { error {Expected four ASYNC_REG synchronizer cells} }",
        ...
        "if {!`$raw_endpoint_ok} { error {fire_done has endpoints outside capture PRE and raw synchronizer D} }",
        "exit"
...
Write-Output "LIDAR_V2_LASER_EXECUTOR_PASS"

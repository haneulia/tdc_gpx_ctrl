...
$WorkRoot = Join-Path $Hdl "tmp/v2_gpx_frame_lane_assembler"
$Work = Join-Path $WorkRoot $Stamp
$Archive = Join-Path $Hdl (
    "signoff_results/sessions/${Stamp}_v2_gpx_frame_lane_assembler")
...
$SimFiles = $CommonFiles + @(
    "$Hdl/system_integration/v2/tb/tb_lidar_gpx_frame_lane_assembler.vhd",
    "$Hdl/system_integration/v2/tb/tb_lidar_gpx_runtime_slope_masks.vhd"
)
$SynthFiles = $CommonFiles + @(
    "$Hdl/system_integration/v2/tb/lidar_gpx_frame_lane_assembler_impl.vhd"
)

$Project = Join-Path $Work "v2_gpx_frame_lane_assembler_vhdl.prj"
$VerilogProject = Join-Path $Work "v2_gpx_frame_lane_assembler_verilog.prj"
...
$SimulationProfiles = @(
    "tb_lidar_gpx_frame_lane_dedicated_150",
    "tb_lidar_gpx_frame_lane_dedicated_200",
    "tb_lidar_gpx_frame_lane_dual_150",
    "tb_lidar_gpx_frame_lane_dual_200",
    "tb_lidar_gpx_frame_lane_fall_off_150",
    ...
    try {
        ...
        foreach ($Top in $SimulationProfiles) {
            ...
            if ((Get-Content -Raw -LiteralPath $SimLog) -notmatch
                "LIDAR_V2_GPX_(FRAME_LANE_ASSEMBLER|RUNTIME_SLOPE_MASKS)_PASS") {
...
if (-not $SkipImplementation) {
    foreach ($Profile in $ImplementationProfiles) {
        ...
        @(
            ...
            "synth_design -top lidar_gpx_frame_lane_assembler_impl -part xc7z020clg484-2 -mode out_of_context -flatten_hierarchy none -generic G_PROC_CLK_MHZ=$($Profile.mhz)",
...
$Scenario = [ordered]@{
    ...
    canonical_order = "logical Chip ascending, STOP ascending"
    output_lanes = @("Rise", "Fall")
...
Write-Output "LIDAR_V2_GPX_FRAME_LANE_ASSEMBLER_PASS"

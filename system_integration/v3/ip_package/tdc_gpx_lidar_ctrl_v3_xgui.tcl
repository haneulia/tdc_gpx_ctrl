# Canonical Vivado XGUI for tdc_gpx_lidar_ctrl_v3_top.

namespace eval ::tglcv3_xgui {
...
proc init_gui {IPINST} {
  ...
  ipgui::add_static_text $IPINST -name clock_help -parent $clocks -text \
    {ASYNC allows either Processing/TDC frequency order. SYNC requires equal values and the same physical clock net. V3 H6 routine sign-off covers 150/200 and 200/150 MHz.}
...
proc update_PARAM_VALUE.G_OUTPUT_WIDTH {PARAM_VALUE.G_OUTPUT_WIDTH} {
  set_property value_validation_list {32 64} ${PARAM_VALUE.G_OUTPUT_WIDTH}
}
...
proc validate_PARAM_VALUE.G_PROC_CLK_MHZ {
  ...
  return [::tglcv3_xgui::set_validation_result \
    ${PARAM_VALUE.G_PROC_CLK_MHZ} \
    [::tglcv3_xgui::validate_clock_contract \
...
foreach name {G_NUM_CHIPS G_RISE_CAPABILITY_MASK G_FALL_CAPABILITY_MASK} {
  set body [format {
    return [::tglcv3_xgui::set_validation_result ${PARAM_VALUE.%s} \
      [::tglcv3_xgui::validate_topology \
...
foreach name {G_RISE_CAPABILITY_MASK G_FALL_CAPABILITY_MASK} {
  ...
  set user_arg PARAM_VALUE.$name
  set body [format {
    set_property value [::tglcv3_xgui::vhdl_bit_string \

...
$Work = Join-Path $RepoRoot ".work\v3_gpx_cell_collector_diff\$Stamp"
$ToolHome = Join-Path $RepoRoot ".work\v3_cell_diff_tool_home"
...
$VhdlFiles = @(
    ...
    (Join-Path $V3Root (
        "tb\tb_lidar_gpx_cell_collector_hls_diff.vhd"))
)
...
$SimulationProfiles = @(
    "tb_lidar_gpx_cell_collector_hls_diff_dedicated_150",
    "tb_lidar_gpx_cell_collector_hls_diff_dedicated_200",
    "tb_lidar_gpx_cell_collector_hls_diff_one_dual_150",
    "tb_lidar_gpx_cell_collector_hls_diff_one_dual_200",
    "tb_lidar_gpx_cell_collector_hls_diff_reduced_150",
    "tb_lidar_gpx_cell_collector_hls_diff_reduced_200",
    "tb_lidar_gpx_cell_collector_hls_diff_all_dual_150",
    "tb_lidar_gpx_cell_collector_hls_diff_all_dual_200"
)

New-Item -ItemType Directory -Force -Path $Work | Out-Null
New-Item -ItemType Directory -Force -Path $ToolHome | Out-Null

# Generated LUTRAM models load their deterministic initialization images by
# relative filename. Keep those images beside the simulator working directory.
...
try {
    ...
    foreach ($Top in $SimulationProfiles) {
        ...
        Write-Host (
            "LIDAR_V3_GPX_CELL_COLLECTOR_DIFF_PROFILE_PASS top=$Top")
    }
}
finally {
    Pop-Location
...
Write-Host "LIDAR_V3_GPX_CELL_COLLECTOR_DIFF_PASS"

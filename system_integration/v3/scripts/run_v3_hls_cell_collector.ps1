...
$ComponentRoot = Join-Path $V3Root "hls\gpx_cell_collector"
$Config = Join-Path $ComponentRoot "hls_config.cfg"
$WorkDir = Join-Path $RepoRoot ".work\v3_hls_cell_collector_component"
$ToolHome = Join-Path $RepoRoot ".work\v3_hls_cell_tool_home"
$Profiles = @("dedicated", "one_chip_dual", "reduced", "all_dual")
...
try {
    if ($Step -eq "csim" -or $Step -eq "all") {
        foreach ($Profile in $Profiles) {
            ...
            Invoke-HlsTool `
                ...
                -Description "V3 HLS Cell Collector C simulation ($Profile)"
            Write-Host "LIDAR_V3_HLS_GPX_CELL_COLLECTOR_CSIM_PROFILE_PASS profile=$Profile"
        }
    }
    ...
    if ($Step -eq "csynth" -or $Step -eq "cosim" -or $Step -eq "all") {
        Invoke-HlsTool `
            ...
            -Description "V3 HLS Cell Collector C synthesis"
    }
    ...
    if ($Step -eq "cosim" -or $Step -eq "all") {
        ...
        foreach ($Profile in $Profiles) {
            ...
            Invoke-HlsTool `
                ...
                -Description "V3 HLS Cell Collector C/RTL co-simulation ($Profile)"

            $CosimReport = Join-Path $WorkDir (
                "hls\sim\report\gpx_cell_collector_hls_cosim.rpt")
            $ReportText = Get-Content -Raw -LiteralPath $CosimReport
            if ($ReportText -notmatch '\|\s*Verilog\|\s*Pass\|') {
                throw "V3 HLS Cell Collector co-simulation did not pass ($Profile)"
            }
            Copy-Item -Force -LiteralPath $CosimReport -Destination (
                Join-Path $ProfileReportDir (
                    "gpx_cell_collector_hls_${Profile}_cosim.rpt"))
            Write-Host "LIDAR_V3_HLS_GPX_CELL_COLLECTOR_COSIM_PROFILE_PASS profile=$Profile"
        }
    }
}
finally {
...
Write-Host "LIDAR_V3_HLS_GPX_CELL_COLLECTOR_${StepName}_PASS"

...
$Work = Join-Path $RepoRoot ".work\v3_gpx_lane_word_formatter_diff\$Stamp"
$ToolHome = Join-Path $RepoRoot ".work\v3_h4_diff_tool_home"
...
$VhdlFiles = @(
    ...
    (Join-Path $V3Root (
        "tb\tb_lidar_gpx_lane_word_formatter_hls_diff.vhd"))
)
...
$SimulationProfiles = @(
    "tb_lidar_gpx_lane_word_formatter_diff_min32_150",
    "tb_lidar_gpx_lane_word_formatter_diff_max32_200",
    "tb_lidar_gpx_lane_word_formatter_diff_mid64_150",
    "tb_lidar_gpx_lane_word_formatter_diff_max64_200"
)
...
try {
    ...
    foreach ($Top in $SimulationProfiles) {
        ...
        Write-Host (
            "LIDAR_V3_GPX_LANE_WORD_FORMATTER_DIFF_PROFILE_PASS top=$Top")
    }
}
finally {
    Pop-Location
...
Write-Host "LIDAR_V3_GPX_LANE_WORD_FORMATTER_DIFF_PASS"

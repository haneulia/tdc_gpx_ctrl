...
$Work = Join-Path $RepoRoot ".work\v3_gpx_frame_assembler_impl\$Stamp"
$ToolHome = Join-Path $RepoRoot ".work\v3_frame_impl_tool_home"
...
if (-not $SkipHlsSynthesis) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "V3 HLS Frame assembler synthesis failed"
    }
}
...
$VhdlFiles = @(
    ...
    (Join-Path $V3Root "tb\lidar_gpx_frame_assembler_hls_impl.vhd")
)
...
try {
    foreach ($Profile in $Profiles) {
        ...
        @(
            ...
            "synth_design -top lidar_gpx_frame_assembler_hls_impl -part xc7z020clg484-2 -mode out_of_context -flatten_hierarchy none -generic G_PROC_CLK_MHZ=$($Profile.Mhz)",
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "V3 HLS Frame assembler implementation failed: $Name"
        }
        ...
        Write-Host (
            "LIDAR_V3_GPX_FRAME_ASSEMBLER_IMPL_PROFILE_PASS profile=$Name")
...
Write-Host "LIDAR_V3_GPX_FRAME_ASSEMBLER_IMPL_PASS"

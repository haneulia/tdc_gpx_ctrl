...
$Work = Join-Path $RepoRoot ".work\v3_gpx_hit_decoder_impl\$Stamp"
$ToolHome = Join-Path $RepoRoot ".work\v3_impl_tool_home"
...
if (-not $SkipHlsSynthesis) {
    & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $HlsRunner -Step csynth
    if ($LASTEXITCODE -ne 0) {
        throw "V3 HLS Hit decoder synthesis failed"
    }
}
...
$VhdlFiles = @(
    ...
    (Join-Path $V3Root "tb\lidar_gpx_hit_decoder_hls_impl.vhd")
)
...
foreach ($Profile in $Profiles) {
    ...
    @(
        ...
        "synth_design -top lidar_gpx_hit_decoder_hls_impl -part xc7z020clg484-2 -mode out_of_context -flatten_hierarchy none -generic G_PROC_CLK_MHZ=$($Profile.Mhz)",
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "V3 HLS Hit decoder implementation failed: $Name"
    }

    $DrcText = Get-Content -Raw -LiteralPath $Drc
    $BlockingDrcCount = [regex]::Matches(
        $DrcText, '(?m)^\|\s*\S+\s*\|\s*(Critical Warning|Error)\s*\|').Count
    ...
    Write-Host "LIDAR_V3_GPX_HIT_DECODER_IMPL_PROFILE_PASS profile=$Name"
...
Write-Host "LIDAR_V3_GPX_HIT_DECODER_IMPL_PASS"

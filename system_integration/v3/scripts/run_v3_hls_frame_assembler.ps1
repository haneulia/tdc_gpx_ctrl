...
$ComponentRoot = Join-Path $V3Root "hls\gpx_frame_assembler"
$Config = Join-Path $ComponentRoot "hls_config.cfg"
$WorkDir = Join-Path $RepoRoot ".work\v3_hls_frame_assembler_component"
$ToolHome = Join-Path $RepoRoot ".work\v3_hls_frame_tool_home"
$Profiles = @(
    "dedicated",
    "one_chip_dual",
    "fall_off",
...
try {
    if ($Step -eq "csim" -or $Step -eq "all") {
        foreach ($Profile in $Profiles) {
            ...
            Invoke-HlsTool `
                ...
                -Description "V3 HLS Frame Assembler C simulation ($Profile)"
            Write-Host (
                "LIDAR_V3_HLS_GPX_FRAME_ASSEMBLER_CSIM_PROFILE_PASS " +
                "profile=$Profile")
        }
    }
    ...
    if ($Step -eq "csynth" -or $Step -eq "cosim" -or $Step -eq "all") {
        Invoke-HlsTool `
            ...
            -Description "V3 HLS Frame Assembler C synthesis"
    }
    ...
    if ($Step -eq "cosim" -or $Step -eq "all") {
        ...
        foreach ($Profile in $Profiles) {
            ...
            Invoke-HlsTool `
                ...
                -Description (
                    "V3 HLS Frame Assembler C/RTL co-simulation ($Profile)")

            $CosimReport = Join-Path $WorkDir (
                "hls\sim\report\gpx_frame_assembler_hls_cosim.rpt")
            $ReportText = Get-Content -Raw -LiteralPath $CosimReport
            if ($ReportText -notmatch '\|\s*Verilog\|\s*Pass\|') {
                throw "V3 HLS Frame Assembler co-simulation failed ($Profile)"
            }
            Copy-Item -Force -LiteralPath $CosimReport -Destination (
                Join-Path $ProfileReportDir (
                    "gpx_frame_assembler_hls_${Profile}_cosim.rpt"))
            Write-Host (
                "LIDAR_V3_HLS_GPX_FRAME_ASSEMBLER_COSIM_PROFILE_PASS " +
                "profile=$Profile")
        }
    }
}
...
Write-Host "LIDAR_V3_HLS_GPX_FRAME_ASSEMBLER_${StepName}_PASS"

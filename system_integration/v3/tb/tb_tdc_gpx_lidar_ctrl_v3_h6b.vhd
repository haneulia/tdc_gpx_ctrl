-- ============================================================================
-- 테스트 자산 목적: V2에서 확정한 CSR/COMMIT/Reg7 물리 적용/AXIS 계약을
-- V3 H6-B1 통합 Top이 그대로 만족하는지 검증한다.
-- 핵심 검증 계약: GPX identity, Reg7 Shadow/Active/Physical, Rise/Fall lane,
-- Hole/측정 시작 기준시점(T0)/Footer, Runtime 직렬화 Return과 레이저 목표
-- 왕복시간, 출력 폭과 비동기 clock 사건 보존을 실제 Top에서 확인한다.
-- 관련 RTL: tdc_gpx_lidar_ctrl_v3_top, V3 H6-A Parent 데이터 경계,
--           V2 유지 CSR/Processing/Echo/TDC 물리 계층.
-- 실행 회귀: run_v3_h6b_integrated_top_diff.ps1
-- 유지보수 주의: assertion의 V2-K05/K06/K12/K13/K14 이름은 V2 Golden 시험
-- 계보를 추적하기 위해 유지한다. V3 판정은 LIDAR_V3_H6B_* marker로 한다.
-- 제품 회귀는 Processing/TDC 150/200 MHz와 200/150 MHz 두 조합만 실행한다.
-- ============================================================================
library ieee;
...
entity tb_tdc_gpx_lidar_ctrl_v3_axis_core is
    generic (
...
end entity tb_tdc_gpx_lidar_ctrl_v3_axis_core;

architecture sim of tb_tdc_gpx_lidar_ctrl_v3_axis_core is
    ...
    begin
        ...
        result.tdc.active_chip_mask := "1111";
    ...
    signal pd_p : std_logic_vector(
        C_CHIPS * C_STOPS_PER_CHIP - 1 downto 0) := (others => '0');
    signal pd_n : std_logic_vector(
        C_CHIPS * C_STOPS_PER_CHIP - 1 downto 0) := (others => '1');
    signal tdc_stop : std_logic_vector(
        C_CHIPS * C_STOPS_PER_CHIP - 1 downto 0);
...
begin
    ...
    u_dut : entity work.tdc_gpx_lidar_ctrl_v3_top
        generic map (
            G_CSR_CLK_MHZ => 100,
    ...
    p_test : process
        ...
        procedure capture_processing_diag(
            constant diag_index : natural;
            variable actual     : out std_logic_vector(31 downto 0);
            constant case_name  : string
        ) is
            variable control_expected : std_logic_vector(31 downto 0) :=
                (others => '0');
            variable control_actual : std_logic_vector(31 downto 0);
            variable complete : boolean := false;
        begin
            assert fn_diag_is_processing(std_logic_vector(to_unsigned(
                diag_index, 8)))
                report case_name & ": index is not owned by Processing"
                severity failure;
            axi_write(fn_ctl_byte_offset(C_CTL_DIAG_INDEX),
                std_logic_vector(to_unsigned(16#100# + diag_index, 32)));
            for attempt in 0 to 400 loop
                axi_read(fn_ctl_byte_offset(C_CTL_DIAG_INDEX),
                    control_actual);
                if control_actual(C_DIAG_VALID_BIT) = '1' then
                    complete := true;
                    exit;
                end if;
            end loop;
            assert complete
                report case_name & ": indexed diagnostic timeout"
                severity failure;
            ...
            assert control_actual = control_expected
                ...
                severity failure;
            axi_read(fn_ctl_byte_offset(C_CTL_DIAG_DATA), actual);
        end procedure capture_processing_diag;
    ...
    begin
        ...
        -- H6-B1 시나리오 A(V2-K12 계보): staging에는 software가 쓴 수동
        -- MTimer가 그대로 보여야
        ...
        -- H6-B1 시나리오 C(V2-K12 계보): 13-bit MTimer 상한을 한 tick
        -- 넘는 요청은 0x33으로
        ...
        for cycle in 0 to 500000 loop
            wait_proc(1);
            all_ififos_drained_v := true;
            for index in 0 to C_CHIPS - 1 loop
                if read_count(index) /= C_WORDS_PER_CHIP or
                   fifo1_fill(index) /= 0 or fifo2_fill(index) /= 0 then
                    all_ififos_drained_v := false;
                end if;
            end loop;
            exit when all_ififos_drained_v;
        end loop;
        for index in 0 to C_CHIPS - 1 loop
        ...
        -- 통합 Top의 CSR indexed portal이 V3 전용 0x27까지 실제로
        -- 라우팅되는지 확인한다. 정상 데이터 경로에서는 H4 formatter fault가
        -- 없어야 하며 0x1A의 Rise/Fall 요약 Bit도 0이어야 한다.
        capture_processing_diag(C_DIAG_GPX_PROC_FAULTS, status_word,
            "V3-H6B formatter summary diagnostic");
        assert status_word(C_DIAG_V3_GPX_RISE_FORMATTER_ANY_BIT) = '0' and
               status_word(C_DIAG_V3_GPX_FALL_FORMATTER_ANY_BIT) = '0'
            report "V3-H6B normal frame raised formatter summary fault"
            severity failure;
        capture_processing_diag(C_DIAG_V3_GPX_FORMATTER_FAULTS,
            status_word, "V3-H6B formatter bitmap diagnostic");
        assert status_word = x"00000000"
            report "V3-H6B normal frame raised formatter bitmap fault"
            severity failure;

        report "LIDAR_V3_H6B_INTEGRATED_TOP_PASS proc_mhz=" &
        ...
        -- H6-B1 자동 비교기가 읽는 단일 행 형식이다. 시간 차이는 모두
        -- Processing clock 수이며 목표 시간만 공통 5 ns tick 단위이다.
        report "LIDAR_V3_H6B_METRIC proc_mhz=" &
            positive'image(G_PROC_CLK_MHZ) & " tdc_mhz=" &
...
entity tb_tdc_gpx_lidar_ctrl_v3_h6b is
    generic (
        G_PROC_CLK_MHZ : positive := 150;
        G_TDC_CLK_MHZ  : positive := 200;
        G_ACTIVE_RETURNS : positive range 1 to 7 := 7;
...
end entity tb_tdc_gpx_lidar_ctrl_v3_h6b;

architecture sim of tb_tdc_gpx_lidar_ctrl_v3_h6b is
begin
    u_core : entity work.tb_tdc_gpx_lidar_ctrl_v3_axis_core
        generic map (
            G_PROC_CLK_MHZ => G_PROC_CLK_MHZ,
            G_TDC_CLK_MHZ => G_TDC_CLK_MHZ,
            G_OUTPUT_WIDTH => 32,
...
entity tb_tdc_gpx_lidar_ctrl_v3_h6b_axis is
    generic (
        G_PROC_CLK_MHZ : positive := 150;
        G_TDC_CLK_MHZ  : positive := 200;
        G_OUTPUT_WIDTH : positive := 32;
...
end entity tb_tdc_gpx_lidar_ctrl_v3_h6b_axis;

architecture sim of tb_tdc_gpx_lidar_ctrl_v3_h6b_axis is
begin
    u_core : entity work.tb_tdc_gpx_lidar_ctrl_v3_axis_core
        generic map (
            G_PROC_CLK_MHZ => G_PROC_CLK_MHZ,

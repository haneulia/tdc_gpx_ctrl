...
// H0-H3 소스 계약 버전이다. AXI payload에 자동 삽입되는 값이 아니므로,
// Release Manifest와 문서가 이 버전을 함께 기록해야 한다.
constexpr std::uint16_t kHlsContractAbiMajor = 3U;
constexpr std::uint16_t kHlsContractAbiMinor = 1U;

namespace limits {

// H0-H3 HLS에 합성되는 최대 구조 용량이다. 제품 Generic과 Runtime Active
...
// H1 decoded Hit, H2 Cell, H3 ordered Lane Cell이 공유하는 edge 의미다.

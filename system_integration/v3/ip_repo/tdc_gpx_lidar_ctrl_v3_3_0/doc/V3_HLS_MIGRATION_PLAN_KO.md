...
| H6-B3B | 보드 독립 범위 완료 | 4-Chip Parent 구현 | 32/64-bit BD/XCI, 핀·IOB·CDC·DRC, 32-bit 배치·배선/Bitstream/XSA와 64-bit 전체 합성 PASS |
| H6-B4 | 다음 | 실물 보드 Sign-off | 40 MHz Tref, 실제 GPX/VDMA/DDR/cache API/Ethernet, 레이저와 선택적 Echo LVDS 계측 |
...
V2의 기능 ABI와 Golden 결과는 비교 기준으로 고정한다. V3가 공유하는 V2
제어·물리 RTL의 결함 또는 타이밍 문제는 재현 TB를 먼저 보강한 뒤 양 버전에
동시에 적용한다. V3의 각 단계는 소스, 독립 테스트, 도구 설정, 결과 문서를 한
Git 체크포인트로 커밋한다. 생성된 HLS 프로젝트는
...
기록한다. H6-B3A의 PS 트랜잭션과 보드 투입 절차는
[`V3_H6B3A_PS_BOARD_PREFLIGHT_KO.md`](V3_H6B3A_PS_BOARD_PREFLIGHT_KO.md)에 기록한다.
H6-B3B의 Parent 구현 증거는
[`V3_H6B3B_PARENT_IMPLEMENTATION_CHECKPOINT_KO.md`](V3_H6B3B_PARENT_IMPLEMENTATION_CHECKPOINT_KO.md)에
기록한다. 실제 AXI VDMA Register 효과, 물리 DDR DMA,
FreeRTOS/PetaLinux cache API와 Ethernet 송신은 H6-B4에서만 닫는다.

...
| `tb_lidar_gpx_acquisition_coordinator.vhd` | 4-Chip ready 축약 파이프라인의 안전성 보존 | ready 상승 1 TDC clock 지연, Run 해제 즉시 차단, 재무장, fanout/merge |
| `tb_lidar_operation_subsystem.vhd` | 중복 CDC 없이 CSR 상태 의미 보존 | Physical/Simulation 상호배제, 동기화 뒤 파생 scheduler 상태 exact compare |
| `validate_v3_l0_parent.tcl` | 저장된 32/64-bit Parent와 생성 IP의 구조 회귀 | BD 연결, Generic, 주소, 핀, 실제 VDMA XCI frame/delay counter 계약 |
| `run_v3_l0_parent_signoff.tcl` | 실제 xc7z020 전체 Parent 구현 Gate | 171 핀, 364 IOB FF, endpoint/waiver 수, CDC/DRC/timing/route, Bitstream/XSA |
...
- `LIDAR_V3_L0_PARENT_IMPL_SIGNOFF_PASS`
...
| VDMA adapter, Frame Store/IRQ 또는 COMMIT progress service | H6-B3A 전체 + H6-B3B Parent 재개방/구현 + H6-B4 실제 ISR/전환 시험 |
| Parent clock/PS/VDMA/HP/XDC | 32/64 재개방 + 64-bit 합성 + 32-bit 대표 배치·배선 |
...
- Parent 32-bit 구현: `.work/v3_parent_signoff/260813_h6b3b_v3_parent_w32_impl3`
- Parent 64-bit 합성: `.work/v3_parent_signoff/260813_h6b3b_v3_parent_w64_synth1`
- Operation 최종 회귀: `signoff_results/sessions/260813_h6b3b_final_operation_v2_operation`
- Coordinator 최종 기능 회귀: `signoff_results/sessions/260813_h6b3b_final_coordinator_v2_gpx_acquisition_coordinator`
- Coordinator 150/200 MHz 구현: `signoff_results/sessions/260813_h6b3b_shot_ready_pipe_rerun_v2_gpx_acquisition_coordinator`
- V2 최종 패키지/OOC 행렬: `signoff_results/sessions/260813_052023_k010_ip_package`
  (`async32`, `async128` Echo Receiver 비활성, `sync64`, 경고 계약 PASS)
...
- TDC-GPX/레이저/모터/설정된 Echo LVDS의 실제 보드 파형 시험
- 물리 IFIFO Drain 도중 Processing abort와 TDC-GPX soft reset을 함께 발생시키는
  통합 command CDC 복구 시험

H6-B3A PS 시험은 보드 독립 실패 주입과 실제 BSP header 컴파일까지 닫는다.
H6-B3B는 Parent Bitstream/IOB/timing까지 닫는다. 실제 XAxiVdma Register 효과와
DDR/cache API 실호출은 H6-B4 보드 시험 범위다.

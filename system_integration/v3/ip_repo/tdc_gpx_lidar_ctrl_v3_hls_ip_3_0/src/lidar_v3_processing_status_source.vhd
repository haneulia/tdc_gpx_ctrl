...
-- V3 Processing-domain 진단 소유자. V2 ABI를 유지하면서 H4 Rise/Fall
-- formatter fault를 GPX_DATA IRQ와 indexed 진단 0x27에 추가한다.
entity lidar_v3_processing_status_source is
    port (
        i_clk          : in  std_logic;
...
end entity lidar_v3_processing_status_source;

architecture rtl of lidar_v3_processing_status_source is
...
begin
    ...
    p_source : process (i_clk, i_rst_n)
        ...
        variable v_rise_formatter_fault : std_logic;
        variable v_fall_formatter_fault : std_logic;
    ...
    begin
        if i_rst_n = '0' then
            ...
            if i_rise_formatter_fault_sticky /= x"00" then
                v_rise_formatter_fault := '1';
            else
                v_rise_formatter_fault := '0';
            end if;
            if i_fall_formatter_fault_sticky /= x"00" then
                v_fall_formatter_fault := '1';
            else
                v_fall_formatter_fault := '0';
            end if;
            ...
            if i_clear_status = '1' then
                ...
                v_gpx_data_warning := i_gpx_context_fault_sticky or
                    v_hit_fault or v_cell_fault or v_frame_fault or
                    v_rise_formatter_fault or v_fall_formatter_fault;
                ...
                when SOURCE_BUILD =>
                    ...
                    case v_index is
                        ...
                        when C_DIAG_GPX_PROC_FAULTS =>
                            ...
                            v_data(
                                C_DIAG_V3_GPX_RISE_FORMATTER_ANY_BIT) :=
                                v_rise_formatter_fault;
                            v_data(
                                C_DIAG_V3_GPX_FALL_FORMATTER_ANY_BIT) :=
                                v_fall_formatter_fault;
                        ...
                        when C_DIAG_V3_GPX_FORMATTER_FAULTS =>
                            v_data(
                                C_DIAG_V3_FORMATTER_RISE_HI downto
                                C_DIAG_V3_FORMATTER_RISE_LO) :=
                                i_rise_formatter_fault_sticky;
                            v_data(
                                C_DIAG_V3_FORMATTER_FALL_HI downto
                                C_DIAG_V3_FORMATTER_FALL_LO) :=

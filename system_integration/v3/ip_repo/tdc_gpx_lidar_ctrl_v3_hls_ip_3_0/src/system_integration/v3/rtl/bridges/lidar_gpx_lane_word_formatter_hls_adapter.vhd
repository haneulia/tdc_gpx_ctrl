...
-- H3?óê?Ñú ?†ï?†¨?êú ?ïú slope?ùò Cell?ùÑ ?è≠ ?èÖÎ¶ΩÏ†Å?ù∏ 32-bit Canonical WordÎ°?
-- Î≥Ä?ôò?ïú?ã§. ÏµúÏ¢Ö 32/64-bit AXI4-Stream Beat Ï°∞Î¶ΩÍ≥®˘ Line ÎßàÏ°◊ÄÎß? ?†ï?†¨??Ä
-- ?õÑ?ã® RTL AXIS Word packerÍ∞Ä ?ã®?èÖ?úºÎ°? ?Üå?ú†?ïú?ã§.
...
architecture rtl of lidar_gpx_lane_word_formatter_hls_adapter is
    ...
    begin
        ...
        result(C_V3_H4_PROFILE_SLOT_COUNT_HI downto
            ...
            std_logic_vector(value.slot_count);
        -- V2?ùò visible_returns ?ïÑ?ìú?äî ?ã§?†ú ?àò?ã† Í∞úÏàòÍ∞Ä ?ïÑ?ãà?ùº ?òÑ?û¨ Face?óê
        -- ?†Å?ö©?êú Runtime ÏßÅÎ†¨?ôî(?†Ñ?ãú) Return ?ä¨Î°? ?àòÎ•®˘ ?Üå?ú†?ïú?ã§.
...
begin
    ...
    -- H3 ?†ï?†¨ FIFO?óê?Ñú H4 HLS ?ûÖ?†• RegisterÍπåÏ°◊Ä ?ù¥?ñ¥ÏßÄ?çò ?è≠?ù¥ ?Å∞
    -- Ï°∞Ìï©/Routing Í≤ΩÎ°úÎ•®˘ Registered skidÎ°? Î∂ÑÎ¶¨?ïú?ã§. Ready?äî Register?êòÎ©°∆
    -- 2-slot Íµ¨Ï°∞?ù¥ÎØÄÎ°? Backpressure Ï§ëÏóê?èÑ Ï≤òÎ¶¨?ú® 1 Event/clock?ùÑ Î≥¥Ï°¥?ïú?ã§.
    ...
    -- abort Î≥µÍµ¨ ?ÉÅ?ÉúÎ•®˘ H3 Ready?óê ÏßÅÏ†ë Ï°∞Ìï© ?ó∞Í≤∞ÌïòÏßÄ ?ïäÍ≥? Register?êú
    -- ?ûÖ?†• ?óà?ö© Ï∞ΩÏúºÎ°? ?†Ñ?ã¨?ïú?ã§. ?†ï?ÉÅ ?ö¥?ö© Ï§ëÏóê?äî Í≥ÑÏÜç 1?ù¥?ùº IIÎ•®˘ Î∞îÍæ∏ÏßÄ
    -- ?ïä?úºÎ©°∆, stale-output Drain Ï¢ÖÎ£å ?í§ ?ïú Cycle ?õÑ ?ïà?†Ñ?ïòÍ≤? ?ã§?ãú ?ó¥Î¶∞Îã§.
    ...
    begin
        if rising_edge(i_clk) then
            if i_rst_n = '0' then
                ...
                -- ?ôÑÎ£åÏ?Ä ?ã§?ùå ?ûÖ?†• ?äπ?ù∏?ù¥ Í∞ôÏ?Ä cycle?ù¥Î©¢• ?Éà ?ò∏Ï∂úÏù¥ ?ö∞?Ñ†?ïú?ã§.
    ...
    u_hls_lane_word_formatter : h4_formatter_ip
        port map (
            ap_clk   => i_clk,
            ap_rst_n => i_rst_n,
            ap_start => '1',

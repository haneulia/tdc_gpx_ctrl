...
검증 범위는 4 Chip x 4 STOP = 16 APD 채널, STOP당 최대 7 Return, GPX 원본
...
flowchart LR
    ...
    LN --> RM
    RM --> RF["Topology-sized result CDC FIFO"]
...
```text
WORDS_PER_IFIFO = STOPS_PER_CHIP * MAX_RETURNS_PER_STOP
DRAIN_CAP_QUADS = ceil(WORDS_PER_IFIFO / 4)
```
...
```text
EVENTS_PER_CHIP =
    2 * STOPS_PER_CHIP * MAX_RETURNS_PER_STOP
    + IFIFO1_DONE + TERMINAL

EVENTS_PER_SHOT = NUM_CHIPS * EVENTS_PER_CHIP
RESULT_FIFO_DEPTH = max(16, next_power_of_two(EVENTS_PER_SHOT))
```

4 Chip x 4 STOP x 7 Return profile에서는 Chip당 58 event, Shot당 232 event이며
FIFO depth는 256이다. XPM memory type은 `auto`로 두어 작은 command FIFO는
distributed RAM을 유지하고 큰 result FIFO는 BRAM으로 추론되게 한다. 사용자가
별도 memory generic을 관리할 필요는 없다.
...
|---|---:|---:|---:|---:|
| Processing 150 / TDC 200 MHz | +0.081 ns | 0 | 0 | 0 |
| Processing 200 / TDC 150 MHz | +0.297 ns | 0 | 0 | 0 |

BRAM 자동 선택 후 자원은 다음과 같다.

| Profile | LUT | LUTRAM | FF | RAMB36 | RAMB18 |
|---|---:|---:|---:|---:|---:|
| 150/200 MHz | 4,229 | 98 | 3,969 | 3 | 2 |
| 200/150 MHz | 4,231 | 98 | 3,965 | 3 | 2 |

이전 distributed-only 구현의 150/200 profile은 LUT 5,272, LUTRAM 962,
FF 4,906이었다. 큰 result FIFO를 BRAM으로 이동해 LUTRAM 864개와 LUT
1,043개를 줄였고, 별도 runtime 변수나 CSR은 추가하지 않았다.

CDC report의 유일한 `CDC-15` 경고는 STOP gateway의 AMD
`xpm_fifo_async` 내부 RAM write/read 구조다. 사용자 RTL 경로의 Critical CDC는
0이다. `PLIO-6`, `RTSTAT-10`, `ZPS7-1`은 pin/PS를 갖지 않는 OOC harness에서만
...
- `signoff_results/sessions/260805_stage5_h3_diag_sim_r7_v2_gpx_acquisition_subsystem`
- `signoff_results/sessions/260805_stage5_h3_bram_impl_r4_v2_gpx_acquisition_subsystem`
- `signoff_results/sessions/260805_stage5_h3_gateway_regr_r1_v2_gpx_event_gateway`
- `signoff_results/sessions/260805_stage5_h3_lane_regr_r1_v2_gpx_acquisition_lane`

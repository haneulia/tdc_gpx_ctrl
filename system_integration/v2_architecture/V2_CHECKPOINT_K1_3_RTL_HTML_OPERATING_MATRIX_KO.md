...
3. RPM, 광학 Shot 간격, 목표거리 왕복시간과 Runtime Return 수를 조합한 운용 판정.
...
| 자산 | 역할 |
|---|---|
| `tb_tdc_gpx_lidar_ctrl_v2_k05.vhd` | 실제 Top에서 목표거리, Return 수, Processing/TDC 클럭 관계와 첫 Shot Line 완료시간 측정 |
...
| 측정값 | 시작 사건 | 끝 사건 | 단위 |
|---|---|---|---|
| `shot_to_start_clks` | Shot 요청 승인 | TDC 측정 시작 신호(T0) | Processing clock |
| `start_to_stop_clks` | TDC 측정 시작 신호(T0) | 목표거리 대기 종료인 STOP_TDC | Processing clock |
...
`TREADY=1`이 동시에 성립한 마지막 Beat를 기준으로 한다. 비동기 clock의 위상차는
Golden에서 1 Processing clock 범위로 허용한다.

외부 TDC-GPX 기준 클럭은 40 MHz이고 Tref는 25 ns이다. Runtime 목표거리 입력은
5 ns ticks이지만 실제 Reg7.MTimer는 25 ns 단위이므로 다음처럼 올림 양자화된다.
...
- 목표거리: `288`, `668`, `1335` 개의 5 ns ticks
...
2. 22개 전체 Top profile의 목표거리 양자화와 HSIZE/VSIZE/STRIDE/Beat 수가 같다.
3. RTL의 STOP->Line 및 Shot->Line 측정값이 Golden의 비동기 1-clock 범위 안에 있다.
4. 18개 topology profile의 슬롯 수와 출력 geometry가 같다.
5. 운용 예제에 의도된 `PASS`와 `CHECK`가 모두 존재한다.

최악의 Golden 예제는 Processing/TDC `200/150 MHz`, Return 7, 1 km,
1200 RPM, 광학 Shot 간격 0.2도이며 `-524 Processing clocks`로 `CHECK`이다.
이는 느린 TDC drain을 숨기지 않고 운용자가 RPM, Return, 거리 또는 각분해능을 다시
선택해야 함을 표시한다.
...
앞의 두 항목과 package/XGUI/OOC 재검사는 K1-4에서 다루고, 실제 DDR/cache/PCB
측정은 Stage L0 보드 통합 범위로 유지한다.

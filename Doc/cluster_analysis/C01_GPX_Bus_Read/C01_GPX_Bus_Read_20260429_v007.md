...
## 18. v007 -> v008 반영 위치 기록

### 사용자 추가 피드백

사용자는 마지막으로 latency, throughput, pipeline 분석을 추가하면 C01 분석을 끝내고 코딩 수정 단계로 갈 수 있을 것 같다고 했다.

### v008 반영 위치

| 사용자 피드백 | v008 반영 위치 | 반영 내용 |
|---|---|---|
| Latency 분석 필요 | C01 v008 `### Latency / Throughput / Pipeline 분석` | `tick0->capture`, `tick0->bus_phy tvalid`, `sk_brsp valid`, `chip_run consume` latency 공식과 후보 clock별 table 추가 |
| Throughput 분석 필요 | C01 v008 `#### Throughput 분리: burst vs non-burst` | burst 상한과 non-burst response serialization을 분리 |
| Pipeline 분석 필요 | C01 v008 `#### Pipeline stage와 backpressure` | `bus_phy -> sk_brsp -> chip_ctrl -> chip_run` pipeline과 backpressure 영향 정리 |
| PPT에도 반영 필요 | C01 v008 PPT Slide 10 | Latency/Throughput/Pipeline 전용 slide 추가 |

### v008 산출물

- `Doc/cluster_analysis/C01_GPX_Bus_Read/C01_GPX_Bus_Read_20260429_v008.md`
- `Doc/cluster_analysis/C01_GPX_Bus_Read/C01_GPX_Bus_Read_20260429_v008.pptx`

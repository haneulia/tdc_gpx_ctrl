...
200 MHz 기준 1 clock은 5 ns이다. 따라서 EF pin 자체는 마지막 read 이후 11.8 ns 안에 올라오지만, `chip_ctrl`이 보는 `EF_sync`는 clock phase와 2-FF latency 때문에 대략 10~20 ns 정도 더 늦어질 수 있다.
...
## 18. v004 -> v005 반영 위치 기록

### 사용자 추가 피드백

사용자는 F-C01-09와 관련해 200 MHz, 즉 5 ns clock 기준으로 `tS-EF max 11.8 ns`를 만족하려면 3 clock이 필요한지 확인했다. 또한 이 판단이 맞다면 `i_tdc_clk`를 200 MHz에서 250 MHz로 변경할 때 GPX IC clock/timing 만족도를 추가 점검해 달라고 요청했다.

### v005 반영 위치

| 사용자 피드백 | v005 반영 위치 | 반영 내용 |
|---|---|---|
| 200 MHz에서 `tS-EF max 11.8 ns`는 3 clock guard가 맞는가? | C01 v005 `#### v005 보완 판단: 200 MHz / 250 MHz에서 tS-EF clock 환산` | raw EF pin guard는 `ceil(11.8/5)=3 clock`이 맞지만, 2-FF 이후 `EF_sync` 관측은 최소 4 clock 이상으로 분리 |
| 250 MHz 변경 시 GPX IC timing 만족도 점검 | C01 v005 `250 MHz 후보 조합` table | `div=1,ticks=5`는 50 MHz readout이라 금지, 40 MHz 이하를 위해 `bus_ticks * bus_clk_div >= 7` 계약 추가 |
| 다음 Cluster에서 확인할 영향성 | C01 v005 `## 14. 검증 체크리스트`, `## 15. 다음 Cluster로 넘길 계약` | `I-C01-09-250MHz` 영향성 ID와 250 MHz clamp/assert 검증 항목 추가 |

### v005 산출물

- `Doc/cluster_analysis/C01_GPX_Bus_Read/C01_GPX_Bus_Read_20260429_v005.md`
- `Doc/cluster_analysis/C01_GPX_Bus_Read/C01_GPX_Bus_Read_20260429_v005.pptx`

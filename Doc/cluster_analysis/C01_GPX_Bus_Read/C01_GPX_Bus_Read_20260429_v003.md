...
데이터시트 p.7의 READ timing을 `i_tdc_clk=200 MHz` clock tick으로 변환할 때는 기준점을 분리해야 한다. v001/v002의 `capture=15 ns` 표기는 `RDN low`가 실제로 발생한 뒤 IOB FF가 data를 capture할 때까지의 시간이다. 사용자가 지적한 `20 ns`는 transaction accept, 즉 `tick0`에서 IOB FF capture까지의 시간으로 보면 맞다.
...
| div | ticks | tick0->capture | RDN low->capture | burst low-edge period | rate | 상태 |
|---:|---:|---:|---:|---:|---:|---|
| 1 | 4 | 15 ns | 10 ns | 20 ns | 50 MHz | illegal, `tV-DR` 보장 전 sample이며 40 MHz도 초과 |
| 1 | 5 | 20 ns | 15 ns | 25 ns | 40 MHz | 데이터시트 최대 readout 관점의 최단 후보 |
| 2 | 4 | 25 ns | 15 ns | 40 ns | 25 MHz | OK |
| 2 | 5 | 35 ns | 25 ns | 50 ns | 20 MHz | OK, 기본 init |
| 3 | 4 | 35 ns | 20 ns | 60 ns | 17 MHz | OK |
...
- `g_OEN_CONNECTED` 또는 동등 generic으로 OEN 연결 여부를 명시한다.
- OEN 미연결/pull-up 운용에서는 `i_oen_permanent`를 내부적으로 무시하거나 금지하고, drain burst timing을 OEN permanent 비의존 방식으로 검증한다.
- OEN Low 고정 운용은 elaboration assertion 또는 configuration error로 막는다. 현재 RTL은 초기 config write가 필수이므로 Low 고정 예외를 정상 운용 모드로 지원하면 안 된다.
...
- [ ] CSR path의 `bus_clk_div >= 2` clamp가 유지되는가?
...
- [ ] `g_OEN_CONNECTED=false` 또는 동등 generic 도입 시 `oen_permanent` 의존 경로가 비활성화되는가?
- [ ] OEN Low 고정 조건은 지원 불가 configuration으로 막히는가?
...
3. `div=1,ticks=5`를 40 MHz 경계값으로 보고, 실제 기본 운용은 현재 CSR clamp/default처럼 이보다 느리게 유지하는 것에 동의하는가?
4. OEN 미연결/pull-up generic은 지원하되 OEN Low 고정은 unsupported로 막는 것에 동의하는가?
5. 데이터시트 기준에서 C02의 최우선 검증 항목을 `EF1/EF2 active HIGH` 기반 empty FIFO read 방지로 잡는 것에 동의하는가?
6. 다음 C02에서 `chip_ctrl/chip_run`의 capture/drain/force_reinit을 이어서 분석해도 되는가?
...
## 18. v003 -> v004 반영 위치 기록

이 절은 v003 검토 이후 사용자 피드백이 v004 어디에 반영되었는지 추적하기 위한 forward trace이다.

### 변경 원인

사용자 피드백:

- PPT에 실제 Timing Diagram이 없어 이해와 소통에 문제가 있다고 지적했다.
- F-C01-01 판단에 동의했고, `tdc_gpx_cfg_pkg`의 `c_BUS_CLK_DIV_MIN=1` 전환이 필요하다고 했다. 코드 영향성은 다음 Cluster에서 진행하되, 현재 모듈에서 처리 가능한 계약과 방안을 요청했다.
- F-C01-02에 대해 Stream out clock port가 없음을 지적했고, Sync/Async mode를 generic으로 결정해야 한다고 했다.
- F-C01-03~F-C01-06 판단에 동의했다.
- F-C01-07에서 OEN pull-down/Low 고정은 회로적으로 없다고 강제하고, 정상 연결과 pull-up/미연결 두 경우의 예외 전략을 요청했다.
- F-C01-08에서 `o_rsp_pending_r` 등록화 시 1 clock 지연 영향성은 다음 Cluster에서 점검하기로 했다.
- F-C01-09에서 `tS-EF max 11.8 ns` 제약의 구조적 의미와 timing 해석을 요청했다.
...
- `Doc/cluster_analysis/C01_GPX_Bus_Read/C01_GPX_Bus_Read_20260429_v004.md`
...
|---|---|---|
| Timing Diagram 필수화 | 운영 프로토콜 v004 `PPT Timing Diagram 필수 규칙`, C01 v004 PPT Slide 3 | 이후 timing 관련 PPT에는 timing diagram 필수 |
| `c_BUS_CLK_DIV_MIN=1` 전환 방향 | C01 v004 `### F-C01-01`의 `v004 보완 판단` | `div=1=>ticks>=5`, `ticks*div>=5` 계약을 함께 강제 |
| Bus_Phy 자체 보호 방안 필요 | C01 v004 `### F-C01-01` | `i_bus_clk_div`를 Bus_Phy에 추가해 ticks와 함께 latch/check하는 권장안 제시 |
| output stream clock port/CDC 전략 | C01 v004 `### F-C01-02` | `i_m_axis_aclk`, `g_STREAM_CLK_MODE=SYNC/ASYNC`, ASYNC FIFO 필요성 기록 |
| OEN 예외 전략 | C01 v004 `### F-C01-07` | `DYNAMIC_CONNECTED`, `PULLUP_OR_NOT_CONNECTED` 두 모드만 지원 |
| `o_rsp_pending_r` 지연 영향성 | C01 v004 `### F-C01-08` | 1 clock 지연 영향성을 C02 추적 항목으로 등록 |
| `tS-EF` 의미 해석 | C01 v004 `### F-C01-09` | 마지막 data read 후 EF set delay와 2-FF sync latency를 drain stop 판단에 연결 |
| 사용자 승인 기록 | C01 v004 `## 16. 사용자 피드백 기록`의 `v004 사용자 검토 반영` | 사용자 확인 필요 1~6 동의 기록 |

PPT 반영 위치:

| v004 PPT 위치 | 반영 내용 |
|---|---|
| Slide 3 | READ/EF Timing Diagram 추가 |
| Slide 9 | F-C01-01~F-C01-09에 대한 v004 판단 요약 |
| Slide 10 | C02로 넘길 영향성 추적 항목 정리 |

### 추적 근거

- `Doc/TDC-GPX-Datasheet.pdf`: p.7 Read Operations, p.8 OEN operations, p.27 data bus 40 MHz transfer rate
- `tdc_gpx_bus_phy.vhd:527-548`, `tdc_gpx_bus_phy.vhd:646-660`, `tdc_gpx_bus_phy.vhd:665-705`
- `tdc_gpx_cfg_pkg.vhd:298-299`
- `tdc_gpx_csr_chip.vhd:846-859`
- `tdc_gpx_top.vhd:46-52`, `tdc_gpx_top.vhd:134-146`

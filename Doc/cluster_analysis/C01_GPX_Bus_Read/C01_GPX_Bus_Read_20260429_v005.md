...
## 18. v005 -> v006 반영 위치 기록

### 사용자 추가 피드백

사용자는 v005의 200 MHz/250 MHz 비교가 `tS-EF` 중심으로 작성되었지만, 실제로 원한 것은 `정상 READ 운용 시퀀스`에서 200 MHz와 250 MHz를 비교하는 것이었다고 지적했다. PPT도 동일하게 정상 READ sequence 비교가 필요하다고 요청했다.

### v006 반영 위치

| 사용자 피드백 | v006 반영 위치 | 반영 내용 |
|---|---|---|
| 정상 READ 운용 시퀀스 기준으로 200 MHz와 250 MHz 비교 필요 | C01 v006 `## 6. 정상 READ 운용 시퀀스`, `### 정상 READ 운용 시퀀스: 200 MHz와 250 MHz 비교` | `tick0->RDN low`, `RDN low->capture`, `tick0->capture`, `tPW-RL`, burst period/rate를 같은 기준으로 비교 |
| 250 MHz 동일 설정과 정상 후보를 비교해야 판단 가능 | C01 v006 `운용상 legal 조합` table | 250 MHz `div=1,ticks=5`는 50 MHz로 illegal, `div=1,ticks=7`은 정상 후보, `div=2,ticks=5`는 보수 후보로 구분 |
| PPT도 같은 기준 필요 | C01 v006 PPT Slide 3 | 정상 READ 200/250 MHz 비교 Timing Diagram/Table 추가 |

### v006 산출물

- `Doc/cluster_analysis/C01_GPX_Bus_Read/C01_GPX_Bus_Read_20260429_v006.md`
- `Doc/cluster_analysis/C01_GPX_Bus_Read/C01_GPX_Bus_Read_20260429_v006.pptx`

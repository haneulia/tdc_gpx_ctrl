...
코드 주석과 `tdc_gpx_cfg_pkg`의 timing table은 다음 공식을 사용한다.
...
| `tPW-RL` | RDN/WRN low pulse width >= 6 ns |
| `tV-DR` | data valid delay after RDN low <= 11.8 ns |
...
WRITE는 FPGA가 `io_d`를 drive한다. 따라서 bus contention 방지를 위해 WRITE 중에는 OEN과 tri-state 제어가 중요하다.
...
## 8. Bus contention 방지 계약

`bus_phy`는 다음 safety invariant를 명시한다.

| Invariant | 설명 | 코드상 구현 |
|---|---|---|
| `INV-1` | WRITE 시 FPGA가 data bus를 drive하고 OEN은 write 방향에 맞춘다 | WRITE entry 및 `ST_WRITE` |
...
### F-C01-02. 문서상 200 MHz 기준이 강하게 박혀 있다

`bus_phy` timing 분석과 `tdc_gpx_cfg_pkg`는 200 MHz, `T_clk=5 ns` 기준이다. 그런데 상위 문서/Top 주석에는 `i_axis_aclk` 150 MHz와 200 MHz 표기가 혼재한다. `bus_phy` 자체는 `i_tdc_clk` 기준이라 직접 문제는 아니지만, 운용 문서가 clock group을 명확히 구분해야 한다.
...
`oen_permanent`는 burst drain에서 GPX가 계속 bus를 drive하는 맥락으로 보인다. 이때 WRITE 요청은 무시되고 simulation warning만 발생한다.
...
3. 다음 C02에서 `chip_ctrl/chip_run`의 capture/drain/force_reinit을 이어서 분석해도 되는가?
...
- TDC-GPX datasheet 원문 재검증은 이번 단계에서 새로 수행하지 않았고, 코드와 `tdc_gpx_cfg_pkg`에 기록된 timing table을 기준으로 해석했다.
- 다음 버전에서는 사용자 피드백에 따라 timing diagram, 검증 waveform 조건, assertion 후보를 더 구체화할 수 있다.

---

## 17. v001 -> v002 반영 위치 기록

이 절은 v001 검토 이후 사용자 피드백이 v002 어디에 반영되었는지 추적하기 위한 forward trace이다.

### 변경 원인

사용자 피드백:

- C01 분석의 기준 문서는 `Doc/TDC-GPX-Datasheet.pdf`이다.
- Bus_Phy에서 샘플링되는 clock은 200 MHz이다.
- 코드 주석이나 기존 timing table보다 데이터시트를 상위 기준으로 삼아야 한다.
...
| v001에서 보완 필요했던 내용 | v002 반영 위치 | 판단 변화 |
|---|---|---|
| 분석 기준 문서가 코드/주석 중심으로 설명됨 | v002 `## 2. 분석 대상 파일`, `## 3. 기준 데이터시트 근거` | 데이터시트를 C01의 1차 기준으로 승격 |
| TDC-GPX bus timing의 데이터시트 근거 부족 | v002 `## 3. 기준 데이터시트 근거`, `## 6. 정상 READ 운용 시퀀스`, `## 7. 정상 WRITE 운용 시퀀스` | p.7 READ/WRITE timing을 명시 |
| OEN 동작의 데이터시트 근거 부족 | v002 `## 9. Bus contention 방지 계약`, `### F-C01-03` | p.8 OEN operations를 근거로 추가 |
| Status pin 의미와 polarity 근거 부족 | v002 `## 12. Status pin synchronizer` | p.11~12 pin description 반영 |
| 16-bit mode 위험 미반영 | v002 `### F-C01-05. 16-bit mode는 현재 RTL 범위 밖이며 차단 정책이 유지되어야 한다` | Reg14[4] 차단 정책을 보호 조건으로 추가 |
| Empty FIFO read 금지 미반영 | v002 `### F-C01-06. Empty FIFO read 금지는 C02의 최우선 검증 조건이다`, `## 15. 다음 Cluster로 넘길 계약` | C02 핵심 인계 조건으로 추가 |
| Bus_Phy clock 기준 명확화 필요 | v002 `## 3. 기준 데이터시트 근거`, `## 6. 정상 READ 운용 시퀀스`, `### F-C01-02` | `i_tdc_clk=200 MHz`, `T_clk=5 ns`로 고정 |
| 사용자 피드백 기록 부족 | v002 `## 16. 사용자 피드백 기록` | v002 피드백 반영 내역 추가 |

PPT 반영 위치:

| v002 PPT 위치 | 반영 내용 |
|---|---|
| Slide 1 | 기준 문서와 200 MHz clock 기준을 cover에 반영 |
| Slide 3 | READ timing을 데이터시트 p.7과 200 MHz tick 기준으로 설명 |
| Slide 4 | WRITE timing과 OEN HIGH 조건 추가 |
| Slide 8 | EF/LF active polarity와 2-FF sync 의미 보완 |
| Slide 9 | 데이터시트 기반 finding으로 재정리 |
| Slide 10 | C02로 넘길 empty FIFO read 금지 계약 추가 |

### 추적 근거

- `Doc/TDC-GPX-Datasheet.pdf`: p.7 Read/Write Operations, p.8 OEN operations, p.11~12 Pin Description, p.51 Bug Report
- `tdc_gpx_bus_phy.vhd:33-52`, `tdc_gpx_bus_phy.vhd:527-548`, `tdc_gpx_bus_phy.vhd:665-705`

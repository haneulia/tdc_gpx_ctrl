...
## 13. v004 -> v005 반영 위치 기록

변경 시각: `2026-04-29 13:20:14 +09:00`

### 사용자 추가 피드백

사용자는 II(Initiation Interval) 분석을 추가하고, 앞으로 모든 결과마다 latency, throughput, pipeline, II를 분석하며 Timing Diagram 또는 Timing Block Diagram으로 Markdown과 PPT에서 이해할 수 있게 만들 것을 요청했다. 또한 모든 문서에 수정 시간까지 포함하도록 운영 규칙에 추가해 달라고 요청했다.

### v005 반영 위치

| 사용자 피드백 | v005 반영 위치 | 반영 내용 |
|---|---|---|
| latency/throughput/pipeline/II 필수 분석 | 운영 프로토콜 v005 `### Latency / Throughput / Pipeline / II 필수 분석 규칙` | 모든 Cluster Markdown/PPT 결과에 latency, throughput, pipeline, II와 근거를 포함 |
| Timing Diagram 또는 Timing Block Diagram 필수화 확대 | 운영 프로토콜 v005 같은 절 | Markdown과 PPT 모두에서 timing/block diagram 또는 table로 이해 가능하게 작성 |
| 문서 수정 시간 기록 | 운영 프로토콜 v005 `### 문서 수정 시간 기록 규칙` | header에 `최종 수정 시간` 추가, forward trace 수정 시 변경 시각 기록 |

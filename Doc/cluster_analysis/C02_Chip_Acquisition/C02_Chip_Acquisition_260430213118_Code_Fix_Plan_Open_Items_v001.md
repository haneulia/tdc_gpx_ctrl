...
Plan v004의 C02 핵심 RTL/TB 보완 항목은 반영되었다. 2026-04-30 23:19:04 기준으로 OP-C02-01부터 OP-C02-06까지 근거 문서와 xsim 로그로 닫혔다.

마지막 잔여 항목이던 `OP-C02-03 config_ctrl/top expected-count CDC integration`은 top 통합 TB에서 물리 IFIFO를 expected보다 1 word 더 많이 채운 상태로 검증했다. READ가 EF까지 가지 않고 expected 개수에서 멈추는 것을 확인했으므로, fire-count final 기반 expected 계약이 `tdc_gpx_top -> config_ctrl -> expected tuple CDC -> chip_ctrl/chip_run` 경로로 닫혔다.
...
| OP-C02-03 | config_ctrl/top expected-count CDC integration 검증 | 반영 | `C02_Chip_Acquisition_260430231904_Config_Top_Expected_CDC_Fix_v001.md`에서 config_ctrl 단품과 top 통합 count-known drain을 검증했다. |
...
| Boundary ID | 현재 판단 |
|---|---|
| VB-C02-01 I-Mode single | close 가능. Quiet/M/Continuous 제외 정책은 문서화됐고, top 통합 I-Mode single expected-count drain이 PASS |
...
| VB-C02-10 Evidence boundary | close 가능. OP-C02-01~06 Markdown/PPT/log 근거가 모두 생성됨 |

## 6. 우선순위 제안

| 우선순위 | 다음 작업 | 이유 |
|---|---|---|
| - | 후속 C03/C04 인계 정리 | C02 내부 OP-C02-01~06 보완 항목은 close 가능. 다음은 output CDC/후속 Cluster 계약으로 넘길 항목 정리 |

## 7. 최종 판단

OP-C02-03까지 반영되면서 C02 내부 보완 검증의 핵심 잔여 위험은 닫혔다. 다음 단계는 C02 closure 문서와 C03/C04 인계 계약을 정리하는 것이다.
...
- 2026-04-30 23:19:04 +09:00: OP-C02-03 config_ctrl/top expected-count CDC integration은 `C02_Chip_Acquisition_260430231904_Config_Top_Expected_CDC_Fix_v001.md` 및 동일 timestamp PPT에 반영했다.
- 반영 코드: `tb_tdc_gpx_top_int.vhd:640`, `tb_tdc_gpx_top_int.vhd:646`, `tb_tdc_gpx_top_int.vhd:711`, `tb_tdc_gpx_top_int.vhd:726`, `tb_tdc_gpx_top_int.vhd:728`, `tb_tdc_gpx_top_int.vhd:747`, `tb_tdc_gpx_top_int.vhd:758`, `tb_tdc_gpx_top_int.vhd:769`, `tb_tdc_gpx_top_int.vhd:836`, `tb_tdc_gpx_top_int.vhd:847`.
- 반영 로그: `xsim_config_ctrl_op_c02_03.log:30`, `xsim_config_ctrl_op_c02_03.log:32`, `xsim_top_int_op_c02_03.log:75`, `xsim_top_int_op_c02_03.log:79`, `xsim_top_int_op_c02_03.log:84..87`.

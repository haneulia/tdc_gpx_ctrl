...
문서 버전: `v006`  
작성일: `2026-04-29`  
최종 수정 시간: `2026-04-29 17:05:00 +09:00`  
...
1. **Review 수신**: 사용자가 `<artifact>_Review_YYYYMMDD_vNNN.md`를 작성하면 Codex는 채팅 응답으로 finding 검증/동의 여부를 보고한다. 단, 즉시 코드 변경/회귀를 시작하지 않는다.
2. **조치 계획 문서 작성 (Plan)**: Codex는 `<artifact>_Plan_YYYYMMDD_vNNN.md`를 같은 Cluster 폴더에 신규 생성한다. Plan 문서에는 다음을 포함한다 — 대응 대상 finding, 동의 근거, 변경 항목 표(대상 파일/요지/추적 위치 예상/영향), 진행 순서, 위험/완화, 검증 방법, 산출물 예상.
3. **사용자 승인**: Plan 문서를 사용자에게 제시하고 명시적 승인 또는 수정 피드백을 받는다. Plan이 수정되면 Plan 문서 자체도 새 버전으로 생성한다.
4. **진행 및 결과 보고서**: 승인된 Plan대로 진행한다. 완료 후 `<artifact>_YYYYMMDD_vNNN.md`(결과) 신규 버전을 생성하고, 직전 결과 버전 + Plan 문서 두 곳에 forward-trace lineage를 추가한다.
...
| 항목 | 의미 |
|---|---|
| 대응 대상 finding 표 | review 문서의 finding ID 별 요지 + 사실관계 검증 결과 |
| Codex 동의/이의 표 | 각 finding 별 동의 여부와 근거 |
| 조치 계획 표 | `#` / 조치 항목 / 대상 파일 / 변경 요지 / 추적 위치 (예상) / 영향 |
| 진행 순서 (Hard Order) | 의존성을 가진 변경의 적용 순서 |
| 위험 / 완화 | 변경이 회귀나 운영에 미칠 수 있는 부작용과 완화책 |
| 검증 방법 | positive / negative 회귀 시나리오 |
| 산출물 예상 | 생성될 파일 경로 |
| 사용자 피드백 기록 | review 채택 사유, 사용자가 강조한 운영 정책 |
| 추적 근거 | 사용자 review 문서, 직전 결과 버전, 운영 프로토콜 |

운영상 의미:

- 코드 변경 또는 회귀는 **반드시** Plan 문서가 cluster_analysis 폴더에 존재하고 사용자 승인을 받은 후에 시작한다.
- 사용자가 채팅으로 “모두 동의해 진행해줘” 등 짧은 승인을 주더라도, Plan 문서는 별도 파일로 생성되어야 한다 (채팅 승인은 Plan 문서 없이 진행할 근거가 되지 않는다).
- Codex는 Plan을 누락하면 “규칙 위반”으로 인지하고, 발견 즉시 retrospective로 Plan을 작성한다 (단, 같은 사이클 내에서는 한 번만 허용하고, Plan 문서 자체에 “retrospective 작성” 사실을 기록한다).

이 규칙의 목적:

- 사용자 review가 잘못 해석되거나 일부만 반영되는 위험을 줄인다.
- 진행 전에 사용자가 변경 범위를 한 번 더 검토할 기회를 보장한다.
- 결과 보고서가 plan 대비 실제 변경의 차이를 추적할 수 있게 한다.
- Cluster 분석이 길어져도 plan/result/review/forward-trace의 4축 history가 보존된다.
...
### 2026-04-29 Review 처리 사이클 규칙 추가 (v006)

사용자는 C01_GPX_Bus_Read_Code_Verify_Review v003 처리 도중 “수정 계획 문서를 왜 규칙에 맞게 생성하지 않은거야?”라고 지적했다. Codex는 사용자 review에 대한 동의 표명과 조치 계획을 채팅으로만 답하고 cluster_analysis 폴더에 별도 plan 문서를 생성하지 않은 사실을 인정하고, retrospective로 `C01_GPX_Bus_Read_Code_Verify_Plan_20260429_v001.md`를 생성했다.

이를 계기로 사용자는 “review 처리 사이클에 plan → 승인 → 진행 → result 패턴 적용을 규칙으로 업데이트”할 것을 요청했다.

본 v006 운영 프로토콜은 다음 규칙을 추가한다.

- `Review 처리 사이클 규칙` (section 5에 추가, v006 신규)

이 규칙은 즉시 효력을 가지며, 향후 모든 사용자 review (`<base>_Review_*.md`) 수신 시 plan 문서 작성을 코드 변경/회귀 진행 전에 반드시 수행한다. C01_GPX_Bus_Read_Code_Verify_Plan v001은 retrospective였으나 향후 사이클은 정상 순서를 지킨다.

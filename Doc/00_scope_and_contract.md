...
- distance_conv (Start# = 0 고정)
- 옵션 A 기본 (5면, 120° FOV, 2400 shots/face)

### 1.2 Phase 2 (별도 설계, 08번 문서)

- CONTINUOUS 측정 모드 (§2.11.2)
- PACKET_SCOPE = 1 (scan_frame 단위)
- Start# 순환 / Start01 / negative remap
- 상시 EF polling drain FSM
- cell cycle 재정의

Phase 1에서 CONTINUOUS 관련으로 구현하는 것은 **구조 확보만**:
- raw_event에 start_num 필드 포함
- cell 바이트 레이아웃에 start_num[] 슬롯 포함
- header에 measurement_mode, start01, start_timer 필드 포함
- SW 시간 복원 pseudocode에 CONTINUOUS 분기 포함
...
| **start_num** | 이 hit가 귀속된 start window 번호. SINGLE_SHOT에서는 항상 0 |
...
|---|---|---|
| `N_CHIPS` | 4 | 병렬 칩 수 |
| `STOPS_PER_CHIP_MAX` | 8 | 칩당 최대 stop 수 |
| `MAX_HITS_PER_STOP` | 8 | cell 내 hit slot 수 |
| `HIT_SLOT_WIDTH` | 16 | hit slot 비트폭 (Zynq-7000=16, MPSoC=17) |
| `TDATA_WIDTH` | 32 | AXI bus 폭 (bits) |
| `CELL_FORMAT` | 0 | 0=Zynq-7000(32B), 1=MPSoC(64B) |

### 3.2 종속 상수 (제너릭으로부터 자동 산출)

| VHDL constant | 산출식 | 기본값 |
|---|---|---|
| `ROWS_PER_FACE_MAX` | N_CHIPS × STOPS_PER_CHIP_MAX | 32 |
| `CELL_SIZE_BYTES` | f(MAX_HITS, HIT_SLOT_WIDTH, CELL_FORMAT) | 32 |
| `HSIZE_MAX` | ROWS_PER_FACE_MAX × CELL_SIZE_BYTES | 1024 |
| `TDATA_BYTES` | TDATA_WIDTH / 8 | 4 |
| `BEATS_PER_PIXEL` | CELL_SIZE_BYTES / TDATA_BYTES | 8 |
| `BEATS_PER_LINE_MAX` | ROWS_PER_FACE_MAX × BEATS_PER_PIXEL | 256 |
...
| `STOPS_PER_CHIP` | 0x01 | R/W | 칩당 활성 stop 수 (2~MAX) |
| `COLS_PER_FACE` | 0x02 | R/W | face당 shot 수 (기본 2400) |
| `PACKET_SCOPE` | 0x04 | R/W | 0=face 단위, 1=scan_frame 단위 |
| `MEAS_MODE` | 0x05 | R/W | 0=SINGLE_SHOT, 1=CONTINUOUS |
| `HIT_STORE_MODE` | 0x06 | R/W | 0=RAW, 1=CORRECTED, 2=DISTANCE |
| `DIST_SCALE` | 0x07 | R/W | 0=1mm ~ 4=2cm |
| `DRAIN_MODE` | 0x08 | R/W | 0=SourceGating, 1=FullDrain+LF |
| `N_DRAIN_CAP` | 0x09 | R/W | Capped Drain 읽기 제한 (0=무제한) |
| `PIPELINE_EN` | 0x0A | R/W | 0=순차, 1=double-buffer pipeline |
| `N_FACES` | 0x0B | R/W | 폴리곤 미러 면 수 (기본 5) |
| `BUS_CLK_DIV` | 0x0C | R/W | bus_clk 정수 분주기 (기본 1) |
| `BUS_TICKS` | 0x0D | R/W | ticks/transaction (3~6, 기본 5) |
| `STOPDIS_OVERRIDE` | 0x0E | R/W | [3:0] SW StopDis 제어, [4] enable |
| `SLICE_TIMEOUT_CLKS` | 0x0F | R/W | lane timeout (sys_clk cycles, 기본 200) |

Status (R/O, 제너릭 상수 노출):

| CSR 이름 | Offset | 의미 |
|---|---|---|
| `HW_VERSION` | 0x70 | HW 버전 (major/minor) |
| `HW_CONFIG` | 0x74 | N_CHIPS, STOPS_MAX, MAX_HITS, HIT_SLOT_WIDTH, TDATA, CELL_FORMAT packed |
| `ROWS_PER_FACE_MAX` | 0x78 | 제너릭 최댓값 (stride/buffer 기준) |
| `CELL_SIZE_BYTES` | 0x7A | 종속 상수 |
| `HSIZE_MAX` | 0x7C | stride 기준 최댓값 |

Status-Live (R/O, 실시간):

| CSR 이름 | Offset | 의미 |
|---|---|---|
| `STATUS` | 0x80 | busy, pipeline_overrun, lane_error_mask 등 |
| `SHOT_SEQ_CURRENT` | 0x84 | 현재 shot_seq |
| `VDMA_FRAME_COUNT` | 0x88 | 완료된 vdma_frame 수 |
| `BIN_RESOLUTION_PS` | 0x90 | Reg7로부터 자동 산출 |
| `K_DIST_FIXED` | 0x94 | 거리 변환 상수 (자동 산출) |
...
process(clk)
    variable v_rows : natural range 2 to ROWS_PER_FACE_MAX;
begin
...
- stops_per_chip > STOPS_PER_CHIP_MAX → MAX
...
### 6.2 Phase 2 = CONTINUOUS, PACKET_SCOPE=1

| 경로 | Phase 2 변경사항 |
|------|-----------------|
| chip_ctrl FSM | 별도 FSM (상시 EF polling, AluTrigger 없음) |
| cell_builder | cell cycle 기준 재정의 (start_num 기반) |
| face_packet_assembler | shot 경계 재정의 |
| header | Start#/Start01 필드 활용 |
| distance_conv | Start#>0 분기, negative remap |
...
| ROWS_PER_FACE_MAX | `ROWS_PER_FACE_MAX` | 제너릭 상수 |
| cell_size | `CELL_SIZE_BYTES` | 종속 상수 |
| hsize_actual | `hsize_actual_reg` | 런타임 가변 |
| HSIZE_MAX | `HSIZE_MAX` | 종속 상수 |
...
- `stops_per_chip` (CSR, 가변)과 `STOPS_PER_CHIP_MAX` (제너릭, 고정)은 **다른 변수**
- `rows_per_face` (런타임, 가변)와 `ROWS_PER_FACE_MAX` (상수)는 **다른 변수**
...
2. **LF block-read**: Phase 2 CONTINUOUS에서 필수 → 08번 문서
3. **fake-read 기법**: Phase 2 CONTINUOUS에서 검토 → 08번 문서
4. **CELL_FORMAT=1 (MPSoC 64B)**: Phase 2에서 설계
5. **PACKET_SCOPE=1**: Phase 2에서 설계
...
| 08 | 08_phase2_continuous_and_scope1.md | CONTINUOUS, SCOPE=1 | Phase 2 설계 |

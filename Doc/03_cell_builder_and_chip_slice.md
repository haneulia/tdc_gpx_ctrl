...
- CONTINUOUS 모드의 cell cycle 재정의 → 08번 문서
...
t_cell = record
    ...
    hit_slot      : array(0 to MAX_HITS_PER_STOP-1) of unsigned(HIT_SLOT_WIDTH-1 downto 0)

    -- per-hit metadata
    start_num     : array(0 to MAX_HITS_PER_STOP-1) of unsigned(7 downto 0)
...
reserved:               2 bit
────────────────────────────
총 payload:           216 bit = 27 bytes → 정렬 32 bytes (CELL_SIZE_BYTES)
```
...
raw_event 도착 시:
  ...
  if hit_counter[row] < MAX_HITS_PER_STOP:
    cell_buffer[row].hit_slot[seq]      = raw_event.raw_hit(HIT_SLOT_WIDTH-1 downto 0)
    cell_buffer[row].start_num[seq]     = raw_event.start_num
...
**HIT_SLOT_WIDTH truncation**: raw_hit는 17-bit이지만 HIT_SLOT_WIDTH=16이면 하위 16-bit만 저장.
